...
CONF_THRESHOLD     = 0.35  # 信頼度閾値(遠方の小さい対象も拾えるよう緩和、stage2分類で誤検出をフィルタ)
CLS_CONF_THRESHOLD = 0.6   # 分類器(crow/not_crow)のcrow判定閾値
MISS_TOLERANCE     = 6     # 追尾中の一時的な見失い/誤判定を何フレームまで無視するか
EMA_ALPHA          = 0.5   # ターゲット座標の指数移動平均の重み(小さいほど平滑化が強い)
...
def run_tracker(simulate=False, no_display=False):
    ...
    pid_pan  = PIDController(kp=0.15, ki=0.001, kd=0.05, output_limit=30.0)
    pid_tilt = PIDController(kp=0.15, ki=0.001, kd=0.05, output_limit=20.0)
